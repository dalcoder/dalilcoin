...
let check_p2sh obday (tosign:Z.t) (beta:md160) s =
  ...
  let rec eval_script_r (tosign:Z.t) bl stk altstk =
	...
	begin
	  match stk with
	  | (l::stkr) -> eval_script_r tosign br ((md256_bytelist (sha256_bytelist l))::stkr) altstk
	  | _ -> raise (Failure ("not enough inputs for SHA256"))
	end
    | (169::br) ->
	begin
	  match stk with
	  | (l::stkr) -> eval_script_r tosign br ((md160_bytelist (hash160_bytelist l))::stkr) altstk
	  | _ -> raise (Failure ("not enough inputs for HASH160"))
	end
    | (170::br) ->
	begin
	  match stk with
	  | (l::stkr) -> eval_script_r tosign br ((md256_bytelist (hash256_bytelist l))::stkr) altstk
...
let verify_gensignat obday e gsg alpha =
  ...
  | P2pkhSignat(Some(x,y),c,sg) ->
	...
	((x0,x1,x2,x3,x4) = alpha2 && verify_signed_big_int e (Some(x,y)) sg,None,None)
      else
	(false,None,None)
  | P2shSignat(scr) ->
      let (i,x0,x1,x2,x3,x4) = alpha in
  ...
  | EndP2pkhToP2pkhSignat(Some(x,y),c,Some(w,z),d,esg,sg) ->
	...
	if ok then
	  ...
	  if (-916116462l, -1122756662l, 602820575l, 669938289l, 1956032577l) = alpha2 then
  ...
  | EndP2pkhToP2shSignat(Some(x,y),c,beta,esg,scr) ->
	...
	let alpha2 = hashval_md160 (if c then (if evenp y then hashpubkeyc 2 xm else hashpubkeyc 3 xm) else hashpubkey xm ym) in
	let ee = md256_big_int (md256_of_bitcoin_message ("endorse " ^ (addr_daliladdrstr (md160_p2sh_addr beta)))) in
	let ok = (x0,x1,x2,x3,x4) = alpha2 && verify_signed_big_int ee (Some(x,y)) esg in 
	if ok then
	  ...
	  let ee = md256_big_int (md256_of_bitcoin_message ("fakeendorsement " ^ (addr_daliladdrstr (md160_p2pkh_addr beta)) ^ " (" ^ (addr_daliladdrstr alpha) ^ ")")) in
	  if ((-629004799l, -157083340l, -103691444l, 1197709645l, 224718539l) = alpha2
		&&
  ...
  | EndP2shToP2pkhSignat(Some(w,z),d,escr,sg) ->
	...
	if verify_signed_big_int e (Some(w,z)) sg then
	  verify_p2sh obday ee (x0,x1,x2,x3,x4) escr
	else
	  (false,None,None)
      else
	(false,None,None)
  | EndP2shToP2shSignat(beta,escr,scr) ->
      ...
      if i = 1 then (* p2sh *)
	let ee = md256_big_int (md256_of_bitcoin_message ("endorse " ^ (addr_daliladdrstr (md160_p2sh_addr beta)))) in
	let (b,mbh,mtm) = verify_p2sh obday ee (x0,x1,x2,x3,x4) escr in
	if b then
